...
func (r *usageLogRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters UsageLogFilters) ([]service.UsageLog, *pagination.PaginationResult, error) {
	...
	if correlationRequestID := strings.TrimSpace(filters.CorrelationRequestID); correlationRequestID != "" {
		conditions = append(conditions, fmt.Sprintf("correlation_request_id = $%d", len(args)+1))
		args = append(args, correlationRequestID)
	}
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		upstreamCyberAccessProgram sql.NullString
		createdAt                  time.Time
	)
	...
	if upstreamCyberAccessProgram.Valid {
		log.UpstreamCyberAccessProgram = &upstreamCyberAccessProgram.String
	}
	...
	if correlationRequestID.Valid {
		log.CorrelationRequestID = correlationRequestID.String
	}
	if groupID.Valid {
		value := groupID.Int64
		log.GroupID = &value
