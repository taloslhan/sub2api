...
type UsageLog struct {
	...
	// OutputTokensPerSecond 为扣除首 Token 等待后的解码速度（tokens/秒），口径：
	// output_tokens × 1000 / (duration_ms - first_token_ms)。仅在 output_tokens > 0、
	// duration_ms > 0、first_token_ms 非空且 duration_ms > first_token_ms 时有值；
	// 样本无效时为 nil 并序列化为 null，不用 0 冒充有效速度。
...
type AdminUsageLog struct {
	...
	// UpstreamCyberAccessProgram is access_programs.cyber from the upstream response.
