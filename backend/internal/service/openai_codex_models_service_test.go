...
func TestMergeGroupConfiguredCodexModelsInjectsSelectedDaybreakForOAuthOnly(t *testing.T) {
	...
	t.Run("oauth selected", func(t *testing.T) {
		...
		require.Len(t, models[0]["service_tiers"], 2)
...
// CAPYBARA-PATCH: Pinned catalogs retain the Daybreak gate and never synthesize account aliases.
func TestMergePinnedCodexModelsPreservesDaybreakGate(t *testing.T) {
	for _, accountType := range []string{AccountTypeOAuth, AccountTypeAPIKey} {
		for _, selection := range []string{openai.DaybreakBlueModelID, "gpt-*"} {
			t.Run(accountType+"/"+selection, func(t *testing.T) {
				svc := &OpenAIGatewayService{accountRepo: codexModelsVisibilityAccountRepo{
					byGroup: map[int64][]Account{782: {{Platform: PlatformOpenAI, Type: accountType}}},
				}}
				group := &Group{ID: 782, Platform: PlatformOpenAI,
					ModelAllowlist: GroupModelAllowlist{Enabled: true, Models: []string{selection}},
				}
				group.CodexModelsManifestConfig.Enabled = true
				for _, body := range []string{`{"models":[]}`, `{"models":[{"slug":"gpt-daybreak-blue-latest"}]}`} {
					manifest := &OpenAIModelsResponse{Body: []byte(body)}
					require.NoError(t, svc.MergeGroupConfiguredCodexModels(context.Background(), group, manifest, ""))
					if accountType == AccountTypeOAuth && selection == openai.DaybreakBlueModelID && body != `{"models":[]}` {
						require.Equal(t, []string{openai.DaybreakBlueModelID}, codexManifestModelSlugs(t, manifest.Body))
					} else {
						require.Empty(t, codexManifestModelSlugs(t, manifest.Body))
					}
				}
