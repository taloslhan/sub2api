...
func (s *OpenAIGatewayService) MergeGroupConfiguredCodexModels(
	...
	allowDaybreakBlue := false
	// CAPYBARA-PATCH: Only explicit Daybreak selection needs account admission in a pinned catalog.
	needsAccountModels := !group.CodexModelsManifestConfig.Enabled
	if group.ModelAllowlistEnabled() {
		for _, modelID := range group.ModelAllowlist.Models {
			if strings.TrimSpace(modelID) == openai.DaybreakBlueModelID {
				needsAccountModels = true
				break
			}
		}
	}
	if needsAccountModels {
		var err error
	...
	// CAPYBARA-PATCH: Pinned catalogs still enforce Daybreak admission without adding account aliases.
...
func configuredCodexSupportsUltrafastServiceTier(modelID string) bool {
	// CAPYBARA-PATCH: Daybreak inherits Sol's tiers without rewriting the request ID.
	return normalizeKnownOpenAICodexModel(openAIModelCapabilityID(modelID)) == "gpt-5.6-sol"
}
